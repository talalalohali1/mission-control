...
const ROLE_BADGES: Record<string, { label: string; color: string }> = {
    "Squad Lead": { label: "LEAD", color: "bg-orange-100 text-orange-700" },
    "Product Analyst": { label: "INT", color: "bg-blue-100 text-blue-700" },
    "Customer Researcher": { label: "SPC", color: "bg-purple-100 text-purple-700" },
    "SEO Analyst": { label: "SPC", color: "bg-green-100 text-green-700" },
    "Content Writer": { label: "SPC", color: "bg-pink-100 text-pink-700" },
    "Social Media": { label: "INT", color: "bg-cyan-100 text-cyan-700" },
    "Designer": { label: "SPC", color: "bg-rose-100 text-rose-700" },
    "Email Marketing": { label: "INT", color: "bg-amber-100 text-amber-700" },
    "Developer": { label: "INT", color: "bg-indigo-100 text-indigo-700" },
    "Documentation": { label: "INT", color: "bg-teal-100 text-teal-700" },
};
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-2">
                {agents.map((agent) => {
                    const badge = ROLE_BADGES[agent.role] || { label: "INT", color: "bg-gray-100 text-gray-600" };
                    return (
                        <div
                            key={agent._id}
                            className="flex items-center gap-3 px-2 py-2.5 rounded-lg hover:bg-mc-border/40 transition-colors cursor-default group"
                        >
                            {/* Avatar */}
                            <div
